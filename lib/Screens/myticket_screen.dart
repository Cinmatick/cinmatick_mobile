  ...
  initState() {
    // at the beginning, all users are shown
     ...
     UserPreference().getUser().then((value) {
      token = value.token;
       ...
       GetDataProvider().getData(HttpService.bookedShows, token).then((response) {
   ...
   void _runFilter(String enteredKeyword) {
    ...
    if (enteredKeyword.isEmpty) {
      ...
      results = _allUsers
          .where((user) =>
              user["reference"].toLowerCase().contains(enteredKeyword.toLowerCase()))
          .toList();
              ...
              ListTile(
                onTap: () {
                  ...
                  // ));
                      ...
                      //   ),
                      // ),  
                          title: textInfo(_foundUsers[index]['reference'], FontWeight.bold, Colors.white, 16, ""),
                          subtitle: 
                          textInfo("No. of Seat "+_foundUsers[index]["number_of_seats"].toString() + "\n" + "Price per seat NGN" + _foundUsers[index]["show"]["price"].toString(), FontWeight.bold, Colors.white, 13, ""),
                          trailing: 
                          textInfo(_foundUsers[index]["show"]["date"] + "\n" + _foundUsers[index]["show"]["time"], FontWeight.bold, Colors.white, 13, ""),
